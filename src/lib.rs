...
pub mod psyclones {
    ...
    /// A high level index of a node in the topology for self.layer_widths and self.layer_nodes.
    #[derive(Clone, Copy)]
    pub struct net_index {
        pub layer: usize,
        pub node: usize,
    }
    /// The index of a node  and the length of the nodes connections in self.tensor.
    #[derive(Clone, Copy)]
    pub struct node_tensor_index{
        pub index: usize,
        pub length: usize,
    }
    /// The index of a connection for self.tensor.
    #[derive(Clone, Copy)]
    pub struct connection_tensor_index{
        pub in_node: node_tensor_index,
        pub target_connection: usize,
    }
    /// an Artificial Neural Network represented as a flattened tensor of layer wise irregular adjacency
    /// matrices (collumn wise ragged). Uses shifts, conditions (ideally branchless) and masking to 
    /// approximate linear functions to improve CPU based Neural Networks and provide a platform for 
    /// both architecture search given finite resources and transfer learning.
    /// e.g.
    /// layer N of a rot_net:
    /// __________
    /// |10|12| 2|
    /// |1 |2 | 3|
    /// |1 |__| 2|
    /// |__|  |__|
    /// layer entries define connections broadcast from a node. (outgoing connections).
    ///
    /// ---
    ...
    pub struct rot_net {
        ...
        // counter for iteration defaults 0 NOTE: NON-ATOMIC (forward propagation is sequential
        // anyways)
        layer_counter: usize, 
        //TODO:
    ...
    impl rot_net {
        ...
        //pub fn add_split_node(&mut self, target_node: node_tensor_index, target_connection: usize) {
    ...
    pub struct connection_tensor_index{
        pub target_node: node_tensor_index,
        pub target_connection: usize,
    }
    /// an Artificial Neural Network represented as a flattened tensor of layer wise irregular adjacency
    ...
    impl rot_net {
        ...
        ///       is 1 in this case).
        pub fn add_split_node(&mut self, input_node: net_index, output_node: net_index) {
        //pub fn add_split_node(&mut self, split_connection: connection_tensor_index) {
        //  let output_node = connection_tensor_index.out_node(self);
        //  let input_node = connection_tensor_index.in_node(self)
            //TODO: indicate which side of a layer a node is on? may need data structure rework
            //      can use iterator and ragged tensor to ensure connections arent cyclic.
            //      how can connections be used to ensure always acyclic? pass in node_tensor_index
            //      with a sub index for connection and split accordingly? how can a connection be
            //      used to reference two nodes?
            let mut rng = rand::thread_rng();
